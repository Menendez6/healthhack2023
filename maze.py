...
# Your existing code for Player, Pen, Treasure, and Enemy classes

def generate_random_maze(width, height):
    maze = []
    for _ in range(height):
        row = ['X' if random.random() < 0.3 else ' ' for _ in range(width)]
        maze.append(''.join(row))
    return maze

def setup_random_maze(width, height):
    level = generate_random_maze(width, height)
    setup_maze(level)
...
def main():
    ...
    setup_random_maze(30, 20)  # Adjust width and height as needed
...
def Starttime():
    ...
    turtle.goto(10, 300)
    turtle.color("red")
    turtle.write(" It's a fake gold!!! In to laggy mode!!!",
                 align="left", font=(10))
    turtle.goto(-50, 300)
    ...
    for treasure in treasures:
        if player.is_collision(treasure):
            ...
            if player.gold == 100:
                ...
                turtle.clear()
                turtle.goto(-50, 300)
                turtle.write("Player Gold:{}".format(player.gold),
                             align="right", font=(0.0000001))
                turtle.goto(2000, 2000)
                treasure.destroy()
                # treasures.remove(Treasure)
                wn.update()
